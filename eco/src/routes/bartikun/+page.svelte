...
<div id="starshine">
  {#each stars as star (star.top + star.left)}
    <div class="template shine {star.size}" style="top: {star.top}%; left: {star.left}%; animation-delay: {star.delay}s;"></div>
  {/each}

<div data-vaul-drawer-wrapper class="h-screen w-screen bg-pinki
  flex flex-col justify-start items-center align-top">
  <Drawer.Root  bind:open={dialogOpen}  shouldScaleBackground>
    <img  src="{img}" alt="img" class="w-auto h-[80vh] ">
